...
| `csi.csv` has more lines than `csi-logger` reported writing, `seq` restarts mid-file, absurd gap counts (millions) | **Two recordings concatenated.** The Pi has no RTC, so the wall clock restarts on every boot and session directory names repeat; `csi-logger` opens `csi.csv` in append mode, so a colliding name silently appends onto the previous recording. Seen for real: `seq 1805 -> 1` at line 938 | Fixed in `session_start.sh`, which now refuses to reuse a session directory and adds a `_2` suffix instead. Set the clock before recording to stop names colliding at all. **Discard any session captured before this fix whose line count disagrees with the logger's own count** — it contains two runs. |
| Records merged mid-line, a `t_us`-sized number appearing in the `seq` column | Something logged on the RX after `app_main` started. Its log line goes out on the same UART as the CSV and corrupts whatever record is mid-transmission | Never enable logging on the RX for a real capture — `esp_log_level_set("*", ESP_LOG_NONE)` must stay in force. A diagnostic build with station join/leave logging caused exactly this |
